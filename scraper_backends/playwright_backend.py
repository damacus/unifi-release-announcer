...
class PlaywrightBackend(ScraperBackend):
    """Playwright-based scraper backend for JavaScript-rendered content."""
    ...
    async def get_latest_release(self) -> Optional[Release]:
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                await page.goto("https://community.ui.com/releases", wait_until="domcontentloaded")
                
                # Wait for content to load (React app needs time)
                await page.wait_for_timeout(5000)  # 5 seconds for React to render
                
                # Find all release links after JavaScript has rendered
                release_links = await page.locator("a[href*='/releases/']").all()
                
                logging.info(f"Found {len(release_links)} release links after JS rendering")
            ...
            logging.error(f"Error scraping releases with Playwright: {e}")
            return None
