...
class TestStateManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_multiple_tags_independent_state(self) -> None:
        ...
        await self.state_manager.set_last_url("unifi-protect", protect_url)
        await self.state_manager.set_last_url("unifi-network", network_url)
    ...
    async def test_update_existing_tag(self) -> None:
        """Test updating URL for existing tag."""
        old_url = "https://example.com/old/123"
        new_url = "https://example.com/new/456"

        await self.state_manager.set_last_url("unifi-protect", old_url)
        await self.state_manager.set_last_url("unifi-protect", new_url)
    ...
    async def test_has_seen_url_false_for_new_tag(self) -> None:
    ...
    async def test_has_seen_url_false_for_different_url(self) -> None:
        ...
        await self.state_manager.set_last_url("unifi-protect", stored_url)
        result = self.state_manager.has_seen_url("unifi-protect", test_url)
        self.assertFalse(result)

    async def test_has_seen_url_true_for_same_url(self) -> None:
    ...
    async def test_corrupted_state_file_handled_gracefully(self) -> None:
    ...
    @patch("builtins.open")
    async def test_save_state_error(self, mock_open: MagicMock, mock_logging_error: MagicMock) -> None:
    ...
    async def test_get_all_tags(self) -> None:
        ...
        await self.state_manager.set_last_url("unifi-protect", "url1")
        await self.state_manager.set_last_url("unifi-network", "url2")
        await self.state_manager.set_last_url("amplifi", "url3")
    ...
    async def test_get_all_tags_empty_state(self) -> None:
